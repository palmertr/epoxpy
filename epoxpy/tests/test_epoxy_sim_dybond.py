...
class TestDyBondBonding(BaseTest):
    ...
    @pytest.mark.long
    @pytest.mark.dybond_bonding
    def test_epoxy_sim_dybond_langevin_harmonic(self, datadir, tmpdir):
        """
        Here we are doing regression testing for the new bonding routine that operates and the mbuild initial structure
        ...
        :return:
        """
        import epoxpy.abc_type_epoxy_langevin_harmonic_simulation as es
        import epoxpy.temperature_profile_builder as tpb
        import epoxpy.bonding as bondClass
        import random
        import os
        ...
        sim_name = 'shrunk_freud_bonding'
        out_dir = os.path.join(out_dir, sim_name)
        myEpoxySim = es.ABCTypeEpoxyLangevinHarmonicSimulation(sim_name, mix_time=mix_time, mix_kt=mix_kt,
                                               temp_prof=type_A_md_temp_profile,
    ...
    @pytest.mark.long
    @pytest.mark.dybond_bonding
    def test_epoxy_sim_dybond_dpdlj(self, datadir, tmpdir):
        """
        Here we are doing regression testing for the new bonding routine that operates and the mbuild initial structure
        whose volume is shrunk to a density of 3.0
        :param datadir:
    ...
    @pytest.mark.long
    @pytest.mark.dybond_bonding
    def test_epoxy_sim_dybond_dpdfene(self, datadir, tmpdir):
        """
        Here we are doing regression testing for the new bonding routine that operates and the mbuild initial structure
        whose volume is shrunk to a density of 3.0
        :param datadir:
